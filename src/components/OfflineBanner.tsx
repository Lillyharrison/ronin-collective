...
export function OfflineBanner() {
  ...
  }, [isSyncing, prevSyncing, isOnline, pendingCount]);
...
// ── Simple usePrevious utility ────────────────────────────────────────────────
function usePrevious<T>(value: T): T | undefined {
  const ref = useState<T | undefined>(undefined);
  // use a ref instead of state to avoid re-render
  const prev = ref[0];
  // We want this to update after render without triggering a re-render
  useEffect(() => {
    ref[1](value); // eslint-disable-line react-hooks/exhaustive-deps
  });
  return prev;
}
